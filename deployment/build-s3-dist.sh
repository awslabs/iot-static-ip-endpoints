...
do_replace "*.template" %%SOLUTION_ID%% "${SOLUTION_ID}"
do_replace "*.template" %%SOLUTION_NAME%% "${SOLUTION_TRADEMARKEDNAME}"
do_replace "*.template" %%SOLUTION_DISPLAY_NAME%% "${SOLUTION_NAME}"
do_replace "*.template" %%VERSION%% "${SOLUTION_VERSION}"
do_replace "*.template" %%SOLUTION_VERSION%% "${SOLUTION_VERSION}"
...
for d in `find . -mindepth 1 -maxdepth 1 -type d`; do
    ...
    if ls $fname/*.py 1> /dev/null 2>&1; then
        ...
        echo "===================================="
        cd $fname
        if [ "$AWS_EXECUTION_ENV" == "CloudShell" ] || [ ! -z "$CODEBUILD_BUILD_ARN" ]; then
            python3.8 -m pip install --upgrade -r requirements.txt -t .
        else
            docker run -v "$(pwd)":/var/task lambci/lambda:build-python3.8 /bin/bash -c "cat requirements.txt; pip3 install --upgrade -r requirements.txt -t .; exit"
        fi
        echo "zipping the artifact"
        zip -r9 $staging_dist_dir/$fname.zip .
    ...
    cp $staging_dist_dir/$fname.zip $build_dist_dir
